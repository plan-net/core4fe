...
function createResizeSensor (el, { value, arg, options }) {
  ...
  switch (arg) {
    ...
    case 'throttle':
      callBack = debounce(
        ...
        options.delay,
        { leading: true, trailing: true, maxWait: options.delay }
      )
      break
  }
  /*
  ACHTUNG
  */
  const resizeEl = el.querySelector('.v-data-table__wrapper > table')
  // We have problems in combination with fixed header
  const res = new ResizeSensor(resizeEl, callBack)
...
function inserted (el, { value, arg, modifiers }, { context: component }) {
  ...
  el.__visibility__listener__ = listenToVisible(el, () =>
    createResizeSensor(el, { value, arg, options })
  )
}
